...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not self.is_double:
            if isinstance(self, FireAnt) and hasattr(self, 'all_damage'):
                self.all_damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while current_place:
            if not current_place.is_hive and current_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        all_damage = amount
        if self.health - amount <= 0 and current_health > 0:
            all_damage += self.damage
        for bee in self.place.bees[:]:
            bee.reduce_health(all_damage)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained is None and other.is_container != True:
            return True
        else:
            return False
        # END Problem 8a
